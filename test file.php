...
$activityEndDateArray=array();// save it later for marking the end
//to storage data in the database and check in the future
	...
	if($resultASD === false) {
		...
		//print_r($activityStartDateArray); //check
...
function draw_calendar($month,$year,$activityStartDateArray = array()){
	//print_r($activityStartDateArray);//the array itself is not empty
	/* draw table */
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		$calendar.= '<td class="calendar-day"><div style="position:relative;height:100px;">';
			...
			//eventdate is a iterater	
			$event_day = $year.'-'.$month.'-'.$list_day;//current date 
			//echo $event_day;
			
			
			
			foreach($activityStartDateArray as $value){
				if($value==$event_day){
					//echo 'max really has guts';
